...
import { goBack, sendHome, checkresponse, validation, sessioncheck,HBRout,priceOnPercent,toastMessage } from './../../Comman';
...
class CreatePrivateContest extends Component {
  ...
  getWinningPrize = () => {
    ...
    if(formthis.state.entryFees<formthis.state.minEntryFees && formthis.state.totalPrizePool !== "0"){
      toastMessage("error","Entry fee can not be less than Rs "+formthis.state.minEntryFees);
      return true;
    }
      var api_url = `${config.API_URL}`;
      var apiUrl = "";
          ...
          if (response.data.error === false) {
            if(response.data.data){
              ...
              formthis.setState({
                ...
                selectedPrizeBreakUp:(response.data.data.winnerslabs && response.data.data.winnerslabs.length>0) ? response.data.data.winnerslabs[0] : {},
                stepNo:2
              },()=>{
                this.setState({
                  selectedBreakUpId : (this.state.selectedPrizeBreakUp)?this.state.selectedPrizeBreakUp._id.$oid:""
                })
  ...
  selectPrizeBrkUp=()=>{
    ...
    $('.prizeBreakUpCheckbox').on('change', function() {
      $('.prizeBreakUpCheckbox').not(this).prop('checked', false);
       id  = $(this).val();
       if(id){
        selectedPrizeBreakUp = {};
        selectedPrizeBreakUp = formthis.searchByKeyArray(id,formthis.state.winnerSlabs)
        formthis.setState({
          stepNo:2,
  ...
  createContest = () => {
    ...
    else
    {
      swal({
        ...
        if (isConfirm) {
          window.location.href = HBRout + '/CreateTeams/' + matchid+'?redirecturl=CreatePrivateContest/' + matchid+'';
        }
        else
        {
  ...
  btnNotification=()=>{
    ...
    {
      window.location.href = HBRout + '/ChoosePrivateTeam/' + matchid + '/' + responsejoin;
    }
      ...
      <div className="fadeIn">
        ...
        <ToastContainer />
          <div className="">
            ...
            <AvForm onValidSubmit={this.getWinningPrize}  className={(this.state.stepNo===1) ? "":"hidden"}>
                ...
                <div className="myprofile_details">
                  ...
                  <div className="profile_fieldsdetails">
                    <div className="form-group">
                      <AvField name="contestName" label="Give your contest name" placeholder="(optional)" onChange={formthis.onChange} type="text"/>
                    </div>
                    <div className="form-group">
                      <AvField type="number" label="Total Prize Pool" min={(this.state.minWinPrize) ? this.state.minWinPrize :"0"} className="form-control" name="totalPrizePool" onKeyUp={this.calculateEntryFees} onChange={formthis.onChange} value={formthis.state.totalPrizePool} placeholder={"min "+this.state.minWinPrize+" & max "+this.state.maxWinPrize}
                      validate={{
                    ...
                    <div className="form-group">
                      <AvField type="number" min={this.state.minContestSize} label="Contest Size" className="form-control" name="contestSize" onKeyUp={this.calculateEntryFees} onChange={formthis.onChange} value={formthis.state.contestSize} placeholder={"min "+this.state.minContestSize+" & max "+this.state.maxContestSize}
                        validate={{
            ...
            <div className={(this.state.stepNo===2) ? "recommended-section":"hidden"}>
              <div className="myContestDetail_mid">
                <div>
                  <div className="contestDetailItemHeader">CONTEST SIZE</div>
                  <div className="contvalue_pluse">{(this.state.contestSize) ? this.state.contestSize :"0"}</div>
                </div>
                <div>
                  <div className="contestDetailItemHeader">TOTAL WINNINGS</div>
                  <div>
                    <span className="contvalue_pluse2">₹ {(this.state.totalPrizePool) ? this.state.totalPrizePool :"0"}</span>
                  </div>
                </div>
                <div>
                  <div className="contestDetailItemHeader">ENTRY FEE</div>
                  <div className="contvalue_pluse3">₹ {(this.state.entryFees) ? this.state.entryFees :"0"}</div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="winnerBreakupSelectorLabel">Choose total no. of winners</div>
                  <div className="winnerBreakupSelectBox" onClick={this.changeWinningBrkUp}>
                    <span className="winnerBreakupSelectText">{(this.state.selectedPrizeBreakUp && this.state.selectedPrizeBreakUp.winner)?this.state.selectedPrizeBreakUp.winner:"0"} Winners <span className="isRecommended">(Recommended)</span></span>
                ...
                <div className="private_tabledatamid">
                  <div className="table-responsive">
                    <table className="table">
                      {
                        (this.state.selectedPrizeBreakUp && this.state.selectedPrizeBreakUp.ranks && this.state.selectedPrizeBreakUp.ranks.length) ?
                        (formthis.state.selectedPrizeBreakUp.ranks).map((item,index)=>{
                          return(
                            <tr key={index}>
                              <td> <strong>{(item.pmin && item.pmax && item.pmin===item.pmax) ? item.pmin : item.pmin+"-"+item.pmax} </strong> </td>
                              <td> {(item.percent) ? item.percent+"%" :""} </td>
                              <td> <i className="fa fa-inr"> </i> {priceOnPercent(formthis.state.totalPrizePool,item.percent)} </td>
                            </tr>
                          )
                ...
                <button onClick={this.createContest} className={"all_transaction up_bt pointer"}>CREATE CONTEST & INVITE FRIENDS</button>
              </div> : ""
              }
            </div>

            <div className={(this.state.stepNo===3) ? "prizeBreakUp":"hidden"}>
              {
                (this.state.winnerSlabs && this.state.winnerSlabs.length>0) ? 
                (this.state.winnerSlabs).map((item,index)=>{
                  return(
                    <div className={(item._id.$oid && item._id.$oid=== formthis.state.selectedBreakUpId) ? "rankprivate_tablenos active" :"rankprivate_tablenos"} key={index}>
                      <table>
                          ...
                          <table>
                            <thead> 
                              <tr className="privatethead_odsinner">
                                ...
                                <label className="custom_bcheckad"> 
                                  ...
                                  <input type="checkbox" className="prizeBreakUpCheckbox" value={(item._id.$oid) ? item._id.$oid :""} checked={(item._id.$oid== formthis.state.selectedBreakUpId) ? "checked" :""}  onChange={formthis.selectPrizeBrkUp}/> 
                            ...
                            </thead>
                            {
                              ...
                              (item.ranks).map((item2,index2)=>{
                                return(
                                  <tr key={index2}>
                                    <td> <strong>{(item2.pmin && item2.pmax && item2.pmin===item2.pmax) ? item2.pmin : item2.pmin+"-"+item2.pmax} </strong> </td>
                                    <td> {(item2.percent) ? item2.percent+"%" :""} </td>
                                    <td> <i className="fa fa-inr"> </i> {priceOnPercent(formthis.state.totalPrizePool,item2.percent)} </td>
                                  </tr>
                                )
